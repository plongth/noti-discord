...
- newsletter delivery mode for WhatsApp `@newsletter` chats:
  ...
  when newsletter media URL sends fail and the source is Discord CDN, retry with buffer payloads; for image sends, also try a JPEG-converted image payload before document-variant and text/link fallback.
