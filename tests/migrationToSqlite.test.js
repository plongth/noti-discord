...
test('one-time migration imports legacy JSON + Baileys files into SQLite and creates backup', async () => {
  ...
  try {
    ...
    await writeLegacyFile(
      ...
      JSON.stringify({ Token: 'TOK', GuildID: 'G', ControlChannelID: 'C', Categories: ['cat'] }),
    );
    await writeLegacyFile(path.join(sandboxDir, 'chats'), JSON.stringify({ '123@s.whatsapp.net': 'chan-1' }));
    await writeLegacyFile(path.join(sandboxDir, 'contacts'), JSON.stringify({ '123@s.whatsapp.net': 'Alice' }));
    await writeLegacyFile(path.join(sandboxDir, 'lastMessages'), JSON.stringify({ dc1: 'wa1', wa1: 'dc1' }));
    ...
    await assert.rejects(() => fs.stat(path.join(sandboxDir, 'settings')), /ENOENT/);
    await assert.rejects(() => fs.stat(path.join(sandboxDir, 'baileys')), /ENOENT/);

    const entries = await fs.readdir(sandboxDir);
    const backupDirs = entries.filter((name) => name.startsWith('legacy-backup-'));
