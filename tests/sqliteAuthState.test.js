...
const withTempStorage = async (fn) => {
  ...
  const tempBase = await fs.mkdtemp(path.join(os.tmpdir(), 'wa2dc-sqlite-auth-'));
...
test('SQLite auth state stores and loads Baileys key namespaces', async () => {
  await withTempStorage(async () => {
    ...
    const appStateKey = await authState.keys.get('app-state-sync-key', ['sync1']);
    ...
    assert.equal(Buffer.from(appStateKey.sync1.keyData).toString('utf8'), 'sync-data');
